  ...
  try {
    ...
    const rows = feats.map((f: any) => {
      ...
      const contribs = coeffs
        .map((c) => ({
          feature: c.feature,
          contribution: Number(((f as any)[c.feature] ?? 0) * c.weight),
        }))
    ...
    const sortedRows = rows.sort((a: any, b: any) => b.prob_points - a.prob_points);
    return Response.json(sortedRows);
  } catch (err: any) {
    console.error("/api/predict error:", err?.message || err);
