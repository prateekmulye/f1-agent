...
class InputValidator:
    ...
    def _compile_patterns(self) -> None:
        ...
        self.code_injection_regex = [
            re.compile(pattern, re.IGNORECASE) for pattern in self.CODE_INJECTION_PATTERNS
        ]
    ...
    def validate(self, user_input: str) -> ValidationResult:
        ...
        if line_count > self.MAX_LINES:
            if self.strict_mode:
                ...
                warnings.append(f"Input has {line_count} lines (max recommended: {self.MAX_LINES})")
        
        # Check for suspicious patterns (prompt injection attempts)
        for pattern in self.suspicious_regex:
            if pattern.search(user_input):
                if self.strict_mode:
                    errors.append("Input contains suspicious patterns that may indicate prompt injection")
                    logger.warning(
                    ...
                    warnings.append("Input contains patterns that may be misinterpreted")
                break
        ...
        special_char_ratio = special_char_count / len(user_input) if len(user_input) > 0 else 0
