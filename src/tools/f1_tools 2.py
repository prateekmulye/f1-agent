...
class QueryF1HistoryInput(BaseModel):
    ...
    query: str = Field(
        description="The search query for historical F1 data"
    )
...
class PredictRaceOutcomeInput(BaseModel):
    ...
    season: int = Field(
        description="Season year for the prediction"
    )
...
def initialize_tools(settings: Settings, vector_store: VectorStoreManager, tavily_client: Optional[TavilyClient] = None) -> None:
    ...
    _tavily_client = tavily_client if tavily_client is not None else TavilyClient(settings)
