...
class IngestionPipelineError(F1SlipstreamError):
...
class IngestionPipeline:
    ...
    async def ingest_incremental(
        ...
        """
        self.logger.info(
            "starting_incremental_ingestion",
            files_to_check=len(file_paths)
        )
        ...
        for file_path in file_paths:
            ...
            try:
                ...
                self.logger.error("file_ingestion_failed", file_path=file_path, error=str(e))
                self._progress["errors"].append(error_msg)
                continue
        ...
        self.logger.info(
            "incremental_ingestion_complete",
            stats=stats
        )
