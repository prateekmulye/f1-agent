...
class DataLoadError(F1SlipstreamError):
    """Exception raised when data loading fails."""
    pass


class DataValidationError(F1SlipstreamError):
...
class DataLoader:
    ...
    def load_json(
        ...
        try:
            ...
            self.logger.error("json_parse_error", file_path=str(file_path), error=str(e))
            raise DataLoadError(f"Failed to parse JSON: {e}") from e
        except Exception as e:
            self.logger.error("json_load_failed", file_path=str(file_path), error=str(e))
