...
class TestRequirement1CurrentData:
    ...
    async def test_1_2_rag_combines_realtime_and_knowledge(
        ...
        vector_docs = [
            Document(
                page_content="Historical context", metadata={"source": "vector_store"}
            )
        ]
...
class TestRequirement2HistoricalData:
    ...
    @pytest.mark.asyncio
    async def test_2_2_retrieval_similarity_threshold(self, mock_vector_store: Mock):
    ...
    async def test_2_3_conversation_context_maintenance(self):
        ...
        assert (
            len(messages) <= 20
        ), f"Should maintain max 20 messages, got {len(messages)}"
...
class TestRequirement4RealtimeSearch:
    ...
    async def test_4_1_query_analysis_within_500ms(self):
        ...
        needs_realtime = any(
            ...
            for keyword in ["latest", "current", "today", "now", "recent"]
        )
...
class TestRequirement7PromptEngineering:
    ...
    def test_7_5_guardrails_for_off_topic(self):
        ...
        assert any(
            ...
            for keyword in ["f1", "formula 1", "racing", "motorsport"]
        )
...
class TestRequirement9ErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_9_1_tavily_fallback_to_knowledge_base(self, mock_vector_store: Mock):
...
class TestRequirement12VectorDatabase:
    ...
    async def test_12_2_metadata_filtering(self, mock_vector_store: Mock):
        ...
        filters = {"year": 2021, "category": "championship", "source": "official"}

        await mock_vector_store.similarity_search("test query", filters=filters)
