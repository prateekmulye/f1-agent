...
def mock_openai_chat() -> Mock:
    ...
    # Mock streaming
    async def mock_astream(*args: Any, **kwargs: Any) -> AsyncGenerator[AIMessage, None]:
...
def mock_pinecone_index() -> Mock:
    ...
    mock.describe_index_stats = Mock(return_value={
        "dimension": 1536,
        "index_fullness": 0.1,
...
def mock_vector_store() -> Mock:
    ...
    mock.similarity_search_with_score = AsyncMock(return_value=[
        (Document(page_content="Test doc", metadata={}), 0.95)
    ])
    return mock
...
def sample_agent_state() -> Dict[str, Any]:
    ...
    return {
        "messages": [
            HumanMessage(content="Who won the last race?")
        ],
